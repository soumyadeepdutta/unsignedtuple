...
router.post("/posts/create", async (req, res) => {
  ...
  return res.json({ success: true, message: `post created with id ${post._id}` });
});
/**
...
router.put("/posts/update/:id", async (req, res) => {
  ...
  await post.save()
  return res.json({ success: true, message: `post with id ${req.params.id} updated!` });
});
/**
...
router.delete("/posts/delete/:id", async (req, res) => {
  ...
  if(!post) return res.status(404).json({ success: false, message: `post with id ${req.params.id} not found!` })
  await post.remove()
  return res.json({ success: true, message: `post with id ${req.params.id} deleted!` });
})
